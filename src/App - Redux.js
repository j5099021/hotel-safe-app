...
import updateSafeStatusApp from './reducers'
import {
  updateSafe
} from './actions'
import './scss/keypad.scss'

let store = createStore(updateSafeStatusApp)
    ...
    if(e.target.value=='CLR'){      
      ...
      if(this.state.currentInput.length==this.pinDigits && this.state.pin!=''){
        if(this.state.pin==this.state.currentInput){
          this.setState((state) => ({ pin: '', currentInput: '' }));
          store.dispatch(updateSafe(false))
        }else{
          this.setState((state) => ({ currentInput: this.invalidMsg }));
        }
      } else if(this.state.currentInput.length==this.pinDigits && this.state.pin==''){
        this.setState((state) => ({ pin: this.state.currentInput, currentInput: '' }));
        store.dispatch(updateSafe(true))
      }else{
        this.setState((state) => ({ currentInput: this.invalidMsg }));
      }
    }else{
    ...
    return (
      <div className="hotelSafe">
        ...
        <div className="column--half flex--column">
          ...
          <LockIndicator isLocked={store.getState().hotelSafe.isLocked} />
        </div>
      </div>
    )
  }
